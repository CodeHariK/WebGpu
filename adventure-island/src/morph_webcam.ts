...
let face: THREE.Mesh, eyeL: THREE.Mesh, eyeR: THREE.Mesh;
...
new GLTFLoader()
    ...
    .load('./src/assets/facecap.glb', (gltf) => {
        ...
        if (influences && head.morphTargetDictionary) {
            for (const [key, value] of Object.entries(head.morphTargetDictionary)) {
                const index = value as number; // Ensure `value` is treated as a number
                gui.add(influences, index, 0, 1, 0.01)
                    .name(key.replace('blendShape1.', ''))
                    .listen();
            }
        } else {
            console.error("morphTargetInfluences or morphTargetDictionary is undefined");
        }
...
function animate() {
    ...
    if (video.readyState >= HTMLMediaElement.HAVE_METADATA) {
        ...
        if (results.facialTransformationMatrixes.length > 0) {
            ...
            const object = scene.getObjectByName('grp_transform') as THREE.Mesh;
        ...
        if (results.faceBlendshapes.length > 0) {
            ...
            for (const blendshape of faceBlendshapes) {
                ...
                const key = blendshapesMap[categoryName];
                const index = face.morphTargetDictionary![key];
