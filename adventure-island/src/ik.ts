...
let scene: THREE.Scene, camera: THREE.PerspectiveCamera, renderer: THREE.WebGLRenderer, orbitControls: OrbitControls, transformControls;
let mirrorSphereCamera;

const OOI: Record<string, any> = {};
let IKSolver: CCDIKSolver;

let stats: Stats, gui: GUI, conf: Record<string, any>;
...
async function init() {
    ...
    const gltf = await gltfLoader.loadAsync('./src/assets/kira.glb');
