...
class CustomMesh extends THREE.Mesh {
    ...
    constructor(geometry: THREE.BufferGeometry, material: THREE.Material) {
        ...
        this.updateLocalTransform = () => {
            // Default implementation (can be overridden later)
...
initJolt().then(function (Jolt) {
    ...
    const createThreeWheel = (constraint: initJolt.VehicleConstraint, wheelIndex: number, body: THREE.Mesh) => {
        // Create wheel mesh
        const joltWheel = constraint.GetWheel(wheelIndex);
        const wheelSetting = joltWheel.GetSettings();
        const wheel = new CustomMesh(new THREE.CylinderGeometry(wheelSetting.mRadius, wheelSetting.mRadius, wheelSetting.mWidth, 20, 1), wheelMaterial);
        body.add(wheel);
    ...
    const carBodySettings = new Jolt.BodyCreationSettings(carShape, new Jolt.RVec3(...bodyPosition),
    ...
    callbacks.GetCombinedFriction = (wheelIndex, tireFrictionDirection, tireFriction, body2, subShapeID2) => {
        let bd = Jolt.wrapPointer(body2, Jolt.Body);
        return Math.sqrt(tireFriction * bd.GetFriction()); // This is the default calculation
    };
    ...
    controllerCallbacks.OnTireMaxImpulseCallback = (wheelIndex, result, suspensionImpulse, longitudinalFriction, lateralFriction, longitudinalSlip, lateralSlip, deltaTime) => {
        let res = Jolt.wrapPointer(result, Jolt.TireMaxImpulseCallbackResult);
        // This is the default calculation which will is pre-filled in result (just repeating it here)
        res.mLongitudinalImpulse = longitudinalFriction * suspensionImpulse;
        res.mLateralImpulse = lateralFriction * suspensionImpulse;
    };
    ...
    updateOnExampleUpdate((time: number, deltaTime: number) => {
        prePhysicsUpdate();
