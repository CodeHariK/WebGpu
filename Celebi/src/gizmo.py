...
class VOXEL_GGT_offset_gizmo(bpy.types.GizmoGroup):
    ...
    def draw_prepare(self, context):
        ...
        for axis, gz in self._handles.items():
            ...
            if axis == 'X':
                gz.matrix_basis = gz.matrix_basis @ Matrix.Translation((0,0,-obj.location.x))
                highlightx = gz.is_highlight
            if axis == 'Y':
                gz.matrix_basis = gz.matrix_basis @ Matrix.Translation((0,0,-obj.location.y))
                highlighty = gz.is_highlight
            if axis == 'Z':
                gz.matrix_basis = gz.matrix_basis @ Matrix.Translation((0,0,-obj.location.z))
        ...
        if hightlight and not self._dragging:
            self._dragging = True
            self._start_locations = {obj.name: obj.location.copy() for obj in context.selected_objects}
    ...
    def set_offset(self, value, axis):
        for obj in bpy.context.selected_objects:
            if obj.name in self._start_locations:
                if axis == 'X':
                    x = round(value - self._start_locations[obj.name].x)
                    obj.location.x = x
                if axis == 'Y':
                    y = round(value - self._start_locations[obj.name].y)
                    obj.location.y = y
                if axis == 'Z':
                    z = round(value - self._start_locations[obj.name].z)
                    obj.location.z = z
