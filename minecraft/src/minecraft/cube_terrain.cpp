...
void MinecraftNode::generate_cube_part_mesh(String name, Vector2i pos, bool height_curve_sampling) {
		...
		for (int x = 0; x < part_size; ++x) {
			...
			for (int dir = 0; dir < 4; dir++) {
				...
				if (nx >= 0 && nx < part_size && nz >= 0 && nz < part_size) {
					neighbor_h = heights[nx + nz * vert_dim];
				}
