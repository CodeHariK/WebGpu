...
void CelesteController::debug_draw_label() {
	if (current_state) {
		...
		String label_text = String(current_state->get_name()) + "\nVel: (" + String::num(v.x, 1) + ", " +
				String::num(v.y, 1) + ", " + String::num(v.z, 1) + ")" + "\nSpeed: " + String::num(speed, 1) +
				" (H: " + String::num(h_speed, 1) + ")" + "\nHover: " + (is_hovering ? "YES" : "NO") +
				" Err: " + String::num(last_spring_error, 2) + "\nRideHeight: " + String::num(ride_height, 2) + " [" +
				String::num(min_ride_height, 2) + " - " + String::num(max_ride_height, 2) + "]";

		DebugManager::get_singleton()->draw_text(
				id, label_text, get_global_position() + Vector3(0, 2.5f, 0), 0.001f, Color(0, 1, 0)
		);
	}
}
