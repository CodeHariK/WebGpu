...
void FolioViewZoom::scroll(double p_value) {
	base_ratio -= p_value * sensitivity;
	base_ratio = CLAMP(base_ratio, 0.0, 1.0);
}

void FolioViewZoom::add_base_ratio(double p_delta) {
	base_ratio += p_delta;
	base_ratio = CLAMP(base_ratio, 0.0, 1.0);
}

void FolioViewZoom::set_toggle_active(bool p_active) {
	if (p_active) {
		toggle -= toggle_last;
		toggle_last = toggle;
	} else {
