...
void TerrainSpline2DOld::_bind_methods() {
	ClassDB::bind_method(D_METHOD("set_max_height", "max_height"), &TerrainSpline2DOld::set_max_height);
	ClassDB::bind_method(D_METHOD("get_max_height"), &TerrainSpline2DOld::get_max_height);
	ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "max_height"), "set_max_height", "get_max_height");

	ClassDB::bind_method(D_METHOD("set_spline_width", "spline_width"), &TerrainSpline2DOld::set_spline_width);
	ClassDB::bind_method(D_METHOD("get_spline_width"), &TerrainSpline2DOld::get_spline_width);
	ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "spline_width"), "set_spline_width", "get_spline_width");

	ClassDB::bind_method(D_METHOD("set_falloff_distance", "falloff_distance"), &TerrainSpline2DOld::set_falloff_distance);
	ClassDB::bind_method(D_METHOD("get_falloff_distance"), &TerrainSpline2DOld::get_falloff_distance);
	ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "falloff_distance"), "set_falloff_distance", "get_falloff_distance");

	ClassDB::bind_method(D_METHOD("set_is_closed", "is_closed"), &TerrainSpline2DOld::set_is_closed);
	ClassDB::bind_method(D_METHOD("get_is_closed"), &TerrainSpline2DOld::get_is_closed);
	ADD_PROPERTY(PropertyInfo(Variant::BOOL, "is_closed"), "set_is_closed", "get_is_closed");

	ClassDB::bind_method(D_METHOD("set_blend_mode", "blend_mode"), &TerrainSpline2DOld::set_blend_mode);
	ClassDB::bind_method(D_METHOD("get_blend_mode"), &TerrainSpline2DOld::get_blend_mode);
	...
	ClassDB::bind_method(D_METHOD("set_interpolation_mode", "mode"), &TerrainSpline2DOld::set_interpolation_mode);
	ClassDB::bind_method(D_METHOD("get_interpolation_mode"), &TerrainSpline2DOld::get_interpolation_mode);
	...
	ClassDB::bind_method(D_METHOD("set_falloff_curve", "falloff_curve"), &TerrainSpline2DOld::set_falloff_curve);
	ClassDB::bind_method(D_METHOD("get_falloff_curve"), &TerrainSpline2DOld::get_falloff_curve);
	...
	ClassDB::bind_method(D_METHOD("get_baked_points_3d"), &TerrainSpline2DOld::get_baked_points_3d);
	ClassDB::bind_method(D_METHOD("get_baked_points_2d"), &TerrainSpline2DOld::get_baked_points_2d);
	ClassDB::bind_method(D_METHOD("get_padded_aabb"), &TerrainSpline2DOld::get_padded_aabb);
	ClassDB::bind_method(D_METHOD("is_point_inside", "point", "polygon"), &TerrainSpline2DOld::is_point_inside);
	ClassDB::bind_method(D_METHOD("distance_to_spline", "point", "polygon"), &TerrainSpline2DOld::distance_to_spline);
	ClassDB::bind_method(D_METHOD("get_target_height", "x", "z", "current_h"), &TerrainSpline2DOld::get_target_height);
	ClassDB::bind_method(D_METHOD("deform_heightmap", "heightmap", "offset"), &TerrainSpline2DOld::deform_heightmap, DEFVAL(Vector2(0, 0)));
	ClassDB::bind_method(D_METHOD("_deform_heightmap_row", "row_idx"), &TerrainSpline2DOld::_deform_heightmap_row);
	ClassDB::bind_method(D_METHOD("_on_curve_changed"), &TerrainSpline2DOld::_on_curve_changed);
	ClassDB::bind_method(D_METHOD("generate_splatmap", "heightmap"), &TerrainSpline2DOld::generate_splatmap);
	ClassDB::bind_method(D_METHOD("generate_scatter_transforms", "heightmap", "count", "min_slope", "max_slope"), &TerrainSpline2DOld::generate_scatter_transforms, DEFVAL(0.0f), DEFVAL(90.0f));
...
TerrainSpline2DOld::TerrainSpline2DOld() {
...
void TerrainSpline2DOld::set_max_height(float p_height) {
	max_height = p_height;
	mark_dirty();
}

float TerrainSpline2DOld::get_max_height() const {
	return max_height;
}

void TerrainSpline2DOld::set_spline_width(float p_width) {
	spline_width = MAX(0.0f, p_width);
	mark_dirty();
}

float TerrainSpline2DOld::get_spline_width() const {
	return spline_width;
}

void TerrainSpline2DOld::set_interpolation_mode(InterpolationMode p_mode) {
	interpolation_mode = p_mode;
	mark_dirty();
}

TerrainSpline2DOld::InterpolationMode TerrainSpline2DOld::get_interpolation_mode() const {
	return interpolation_mode;
}

void TerrainSpline2DOld::set_falloff_distance(float p_dist) {
	falloff_distance = MAX(0.0f, p_dist);
	mark_dirty();
}

float TerrainSpline2DOld::get_falloff_distance() const {
	return falloff_distance;
}

void TerrainSpline2DOld::set_is_closed(bool p_closed) {
	is_closed = p_closed;
	mark_dirty();
}

bool TerrainSpline2DOld::get_is_closed() const {
	return is_closed;
}

void TerrainSpline2DOld::set_blend_mode(BlendMode p_mode) {
	blend_mode = p_mode;
	mark_dirty();
}

TerrainSpline2DOld::BlendMode TerrainSpline2DOld::get_blend_mode() const {
	return blend_mode;
}

void TerrainSpline2DOld::set_falloff_curve(const Ref<Curve> &p_curve) {
	falloff_curve = p_curve;
	mark_dirty();
}

Ref<Curve> TerrainSpline2DOld::get_falloff_curve() const {
	return falloff_curve;
}

PackedVector3Array TerrainSpline2DOld::get_baked_points_3d() const {
	Ref<Curve3D> curve = get_curve();
	if (curve.is_null()) {
		return PackedVector3Array();
	}

	TerrainSpline2DOld *mutable_this = const_cast<TerrainSpline2DOld *>(this);
...
PackedVector2Array TerrainSpline2DOld::get_baked_points_2d() const {
...
Rect2 TerrainSpline2DOld::get_padded_aabb() const {
...
void TerrainSpline2DOld::mark_dirty() {
	is_dirty = true;
	emit_signal("spline_changed");
}

void TerrainSpline2DOld::_on_curve_changed() {
	mark_dirty();
}

bool TerrainSpline2DOld::is_point_inside(const Vector2 &p, const PackedVector2Array &polygon) const {
...
float TerrainSpline2DOld::get_distance_to_segment(const Vector2 &p, const Vector2 &a, const Vector2 &b) const {
	Vector2 ab = b - a;
...
float TerrainSpline2DOld::distance_to_spline(const Vector2 &p, const PackedVector2Array &polygon) const {
	int num_pts = polygon.size();
	if (num_pts == 0)
		return 0.0f;
	if (num_pts == 1)
...
TerrainSpline2DOld::SplineEval TerrainSpline2DOld::_evaluate_spline_point(const Vector2 &p, const PackedVector3Array &poly3d, const PackedVector2Array &poly2d) const {
	SplineEval res;
	res.distance = 1e20f;
	res.spline_y = 0.0f;
	res.is_inside = false;
	...
	// LOOP 1: Evaluate the Line Segments (Required for Falloff and IDW Line)
	int limit = is_closed ? num_pts : num_pts - 1;
	for (int i = 0; i < limit; ++i) {
		Vector3 a = poly3d[i];
		...
		float seg_y = a.y + t * (b.y - a.y);

		// Always find the closest physical distance to the shape boundary
		if (dist < min_dist) {
			min_dist = dist;
			closest_y = seg_y;
		}

		// If IDW Line is chosen, accumulate the line weights
		if (interpolation_mode == INTERP_IDW_LINE) {
			float weight = 1.0f / (dist * dist + 0.001f); // +0.001 to prevent division by zero
	...
	// Final Y-Height determination based on the Inspector Dropdown
	if (interpolation_mode == INTERP_NEAREST) {
		...
		// LOOP 2: Only calculate Vertex distances if strictly requested
		float total_weight_vert = 0.0f;
		float blended_y_vert = 0.0f;

		for (int i = 0; i < num_pts; ++i) {
			Vector2 pt_2d(poly3d[i].x, poly3d[i].z);
			float dist = p.distance_to(pt_2d);
			float weight = 1.0f / (dist * dist + 0.001f);
			blended_y_vert += poly3d[i].y * weight;
...
float TerrainSpline2DOld::get_target_height(float x, float z, float current_h) const {
	...
	switch (blend_mode) {
		case BLEND_ADD:
			return current_h + (target_spline_h * weight);
		case BLEND_SUBTRACT:
			return current_h - (target_spline_h * weight);
		case BLEND_MAX:
			return Math::max(current_h, (float)local_lerp(current_h, target_spline_h, weight));
		case BLEND_MIN:
			return Math::min(current_h, (float)local_lerp(current_h, target_spline_h, weight));
		case BLEND_REPLACE:
			return local_lerp(current_h, target_spline_h, weight);
	}
	return current_h;
}

void TerrainSpline2DOld::deform_heightmap(const Ref<TerrainHeightmapOld> &p_heightmap, const Vector2 &p_offset) {
...
void TerrainSpline2DOld::_process(double delta) {
...
void TerrainSpline2DOld::_deform_heightmap_row(int p_row_idx) {
	...
	for (int x = temp_min_x; x <= temp_max_x; ++x) {
		...
		// If the point is outside the spline's influence entirely, skip it immediately
		if (weight <= 0.0f)
			continue;
...
Ref<Image> TerrainSpline2DOld::generate_splatmap(const Ref<TerrainHeightmapOld> &p_heightmap) const {
	Ref<Image> splatmap;
...
TypedArray<Transform3D> TerrainSpline2DOld::generate_scatter_transforms(const Ref<TerrainHeightmapOld> &p_heightmap, int p_count, float p_min_slope, float p_max_slope) const {
