...
class TerrainManagerOld;

class TerrainHeightmapOld : public RefCounted {
	GDCLASS(TerrainHeightmapOld, RefCounted)

private:
	int width = 0;
	int height = 0;
	PackedFloat32Array data;
	TerrainManagerOld *manager = nullptr;
...
public:
	...
	void set_manager(TerrainManagerOld *p_manager, const Vector2i &p_coords);
...
class TerrainSpline2DOld : public Path3D {
	GDCLASS(TerrainSpline2DOld, Path3D)
...
public:
	...
	// Phase 3 Methods
	void deform_heightmap(const Ref<TerrainHeightmapOld> &p_heightmap, const Vector2 &p_offset = Vector2(0, 0));
	...
	// Phase 5 Methods
	Ref<Image> generate_splatmap(const Ref<TerrainHeightmapOld> &p_heightmap) const;
	TypedArray<Transform3D> generate_scatter_transforms(const Ref<TerrainHeightmapOld> &p_heightmap, int p_count, float p_min_slope = 0.0f, float p_max_slope = 90.0f) const;
...
private:
	...
	Ref<TerrainHeightmapOld> temp_heightmap;
	int temp_min_x = 0;
	int temp_max_x = 0;
	int temp_min_z = 0;
	int temp_max_z = 0;
...
class TerrainManagerOld : public Node3D {
	GDCLASS(TerrainManagerOld, Node3D)

private:
	int chunk_size = 64;
	HashMap<Vector2i, Ref<TerrainHeightmapOld>> chunks;
...
public:
	...
	Ref<TerrainHeightmapOld> get_chunk(const Vector2i &p_coords);
	Ref<TerrainHeightmapOld> create_chunk(const Vector2i &p_coords);
