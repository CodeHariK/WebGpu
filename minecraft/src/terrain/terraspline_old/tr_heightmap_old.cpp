...
void TerrainHeightmapOld::_bind_methods() {
	ClassDB::bind_method(D_METHOD("initialize", "width", "height", "default_value"), &TerrainHeightmapOld::initialize, DEFVAL(0.0f));
	ClassDB::bind_method(D_METHOD("get_width"), &TerrainHeightmapOld::get_width);
	ClassDB::bind_method(D_METHOD("get_height"), &TerrainHeightmapOld::get_height);
	ClassDB::bind_method(D_METHOD("get_height_at", "x", "z"), &TerrainHeightmapOld::get_height_at);
	ClassDB::bind_method(D_METHOD("set_height_at", "x", "z", "value"), &TerrainHeightmapOld::set_height_at);
	ClassDB::bind_method(D_METHOD("get_data"), &TerrainHeightmapOld::get_data);
	ClassDB::bind_method(D_METHOD("set_data", "data"), &TerrainHeightmapOld::set_data);
	ClassDB::bind_method(D_METHOD("import_from_image", "image"), &TerrainHeightmapOld::import_from_image);
	ClassDB::bind_method(D_METHOD("export_to_image"), &TerrainHeightmapOld::export_to_image);
	ClassDB::bind_method(D_METHOD("generate_mesh", "lod_level"), &TerrainHeightmapOld::generate_mesh, DEFVAL(0));
	ClassDB::bind_method(D_METHOD("generate_collision_shape"), &TerrainHeightmapOld::generate_collision_shape);
...
TerrainHeightmapOld::TerrainHeightmapOld() {
	width = 0;
	height = 0;
}
...
void TerrainHeightmapOld::initialize(int p_width, int p_height, float p_default_value) {
...
void TerrainHeightmapOld::set_manager(TerrainManagerOld *p_manager, const Vector2i &p_coords) {
	manager = p_manager;
	chunk_coords = p_coords;
}

float TerrainHeightmapOld::get_height_at(int x, int z) const {
	...
	if (x < 0 || x >= width || z < 0 || z >= height) {
		if (manager != nullptr) {
			...
			if (c_size > 0) {
				...
				Ref<TerrainHeightmapOld> neighbor = manager->get_chunk(Vector2i(target_cx, target_cz));
...
void TerrainHeightmapOld::set_height_at(int x, int z, float val) {
	if (x < 0 || x >= width || z < 0 || z >= height) {
		return;
	}
...
void TerrainHeightmapOld::set_data(const PackedFloat32Array &p_data) {
	data = p_data;
...
void TerrainHeightmapOld::import_from_image(const Ref<Image> &p_image) {
...
Ref<Image> TerrainHeightmapOld::export_to_image() const {
	Ref<Image> img;
	img.instantiate();
	if (width <= 0 || height <= 0) {
		return img;
...
Ref<ArrayMesh> TerrainHeightmapOld::generate_mesh(int p_lod_level) const {
	if (width <= 1 || height <= 1) {
...
Ref<HeightMapShape3D> TerrainHeightmapOld::generate_collision_shape() const {
