...
void MPManager::setup_ui() {
	...
	ui.help_dialog = ui.manager->add_dialog(ui.manager, "Marching Prisms Info",
											"This project visualizes the 64 configurations of the Marching Prisms algorithm.\n\n"
											"- Base meshes generate all 64 variants via 120-degree Y-axis rotations.\n"
	...
	mode_btn->set_name("PlacementModeButton");
...
void MPManager::_on_toggle_placement_mode() {
	cancel_drag();
	...
	UtilityFunctions::print("MPManager: Interaction mode toggled to ", mode_name);
...
void MPManager::_on_gui_input(const Ref<InputEvent> &p_event) {
...
void MPManager::_on_show_help() {
	if (ui.help_dialog) {
		ui.help_dialog->popup_centered();
	}
}

void MPManager::update_ui() {
	...
	if (terrain_node && terrain.stats_label) {
		terrain.stats_label->set_text("MP Meshes: " + String::num_int64(terrain_node->get_total_mp_meshes()) +
									  "\nMP Cells: " + String::num_int64(terrain_node->get_total_cells()) +
	...
	if (ui.side_panel && ui.side_panel->is_visible() && ui.variant_stats_vbox) {
		...
		if (mp_node) {
			...
			for (int h = 0; h < 64; h++) {
				String base_name = MPNode::hash_to_binary(h);
