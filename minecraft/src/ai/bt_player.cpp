...
void BTPlayer::_bind_methods() {
	...
	ADD_PROPERTY(
			PropertyInfo(Variant::OBJECT, "bt_tree", PROPERTY_HINT_RESOURCE_TYPE, "BTTask"), "set_bt_tree",
			"get_bt_tree"
	);
	ADD_PROPERTY(
			PropertyInfo(Variant::OBJECT, "btstore", PROPERTY_HINT_RESOURCE_TYPE, "btstore"), "set_btstore",
			"get_btstore"
	);
	ADD_PROPERTY(PropertyInfo(Variant::BOOL, "active"), "set_active", "is_active");
}

BTPlayer::BTPlayer() { btstore.instantiate(); }

BTPlayer::~BTPlayer() {}

void BTPlayer::set_bt_tree(const Ref<BTTask> &p_tree) { bt_tree = p_tree; }

Ref<BTTask> BTPlayer::get_bt_tree() const { return bt_tree; }

void BTPlayer::set_btstore(const Ref<BTStore> &p_btstore) { btstore = p_btstore; }

Ref<BTStore> BTPlayer::get_btstore() const { return btstore; }

void BTPlayer::set_agent(Node *p_agent) { agent = p_agent; }

Node *BTPlayer::get_agent() const { return agent; }
