...
void WheelConfig::_bind_methods() {
	...
	ADD_PROPERTY(
			PropertyInfo(Variant::FLOAT, "suspension_rest_length"), "set_suspension_rest_length",
	...
	ADD_PROPERTY(
			PropertyInfo(Variant::FLOAT, "suspension_stiffness"), "set_suspension_stiffness", "get_suspension_stiffness"
	);
	...
	ADD_PROPERTY(
			PropertyInfo(Variant::FLOAT, "compression_damping"), "set_compression_damping", "get_compression_damping"
	);
...
void WheelConfig::set_hardpoint_offset(const Vector3 &p_offset) { hardpoint_offset = p_offset; }

Vector3 WheelConfig::get_hardpoint_offset() const { return hardpoint_offset; }

void WheelConfig::set_radius(float p_radius) { radius = p_radius; }

float WheelConfig::get_radius() const { return radius; }

void WheelConfig::set_suspension_rest_length(float p_length) { suspension_rest_length = p_length; }

float WheelConfig::get_suspension_rest_length() const { return suspension_rest_length; }

void WheelConfig::set_suspension_stiffness(float p_stiffness) { suspension_stiffness = p_stiffness; }

float WheelConfig::get_suspension_stiffness() const { return suspension_stiffness; }

void WheelConfig::set_compression_damping(float p_damping) { compression_damping = p_damping; }

float WheelConfig::get_compression_damping() const { return compression_damping; }

void WheelConfig::set_rebound_damping(float p_damping) { rebound_damping = p_damping; }

float WheelConfig::get_rebound_damping() const { return rebound_damping; }
