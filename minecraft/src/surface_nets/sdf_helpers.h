...
public:
	...
	inline float sample(const Vector3 &p) const { return p.distance_to(center) - radius; }
};
...
public:
	SDFBox(const Vector3 &p_center,
		   const Vector3 &p_half_size) :
			center(p_center),
	...
	inline float sample_exact(const Vector3 &p) const {
		...
		Vector3 local_p(std::abs(p.x - center.x), std::abs(p.y - center.y), std::abs(p.z - center.z));
		...
		Vector3 max_d(std::max(d.x, 0.0f), std::max(d.y, 0.0f), std::max(d.z, 0.0f));
