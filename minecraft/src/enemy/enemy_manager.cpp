...
EnemyManager::EnemyManager() { singleton = this; }
...
EnemyManager *EnemyManager::get_singleton() { return singleton; }

void EnemyManager::register_enemy(
		Node3D *p_node,
		const String &p_kind
) {
...
Node3D *EnemyManager::get_best_target(
		Vector3 p_origin,
		Vector3 p_input_dir,
		float p_max_range
) {
