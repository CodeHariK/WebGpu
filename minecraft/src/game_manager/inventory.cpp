...
int Inventory::get_item_quantity(const String &p_name) const { return items.get(p_name, 0); }

bool Inventory::has_item(
		const String &p_name,
		int p_min_quantity
) const {
	int current = items.get(p_name, 0);
...
void Inventory::set_items(const Dictionary &p_items) { items = p_items; }

Dictionary Inventory::get_items() const { return items; }

String Inventory::to_json() const { return JSON::stringify(items); }
