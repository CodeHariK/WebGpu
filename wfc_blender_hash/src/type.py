...
class Wfc_blender_hashData(PropertyGroup):
...
def wfc_blender_hash() -> Wfc_blender_hashData:
    wm = bpy.context.window_manager
    if not hasattr(wm, "wfc_blender_hash_data"):
        raise RuntimeError("Wfc_blender_hashData not initialized. Register the addon first.")
    return wm.wfc_blender_hash_data
...
classes = (VoxelItem, HashItem, LibraryItem, Wfc_blender_hashData)
...
def register():
    ...
    WindowManager.wfc_blender_hash_data = PointerProperty(type=Wfc_blender_hashData)
...
def unregister():
    ...
    del WindowManager.wfc_blender_hash_data
